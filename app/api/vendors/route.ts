...
import listVendors from "app/vendors/queries/listVendors"
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await listVendors(null, createMockContext(mockSession))
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Call the Blitz RPC resolver
    const result = await createVendor(body, createMockContext(mockSession))
